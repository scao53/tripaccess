...
others are foreign keys. The arrows show how the datasets are connected.
In the person table, the primary key is person_id and the foreign key is
household_id. In the house table, the primary key is household_id. In
the trip table, the foreign keys are person_id and household_id. The
person and trip tables are connected by person_id and household_id. The
house and person tables are connected by household_id. The house and
trip tables are connected by
household_id.](reference/figures/datarelationships.png)
